...
Description: Module provides facility for staff to send followup SMS to Leads at scheduled time. This module uses default SMS gateways enabled in AB Solutions.
...
define('SI_LEAD_FOLLOWUP_VALIDATION_URL','http://www.absiltdigital.com/absolutions_validation/index.php');
define('SI_LEAD_FOLLOWUP_KEY','c2lfbGVhZF9mb2xsb3d1cA==');

update_option('si_lead_followup_activated', 1);
update_option('si_lead_followup_activation_code', '********************');
$CI = &get_instance();
...
hooks()->add_action('settings_tab_footer','si_lead_followup_hook_settings_tab_footer');#for AB Solutions low version V2.4 
hooks()->add_action('settings_group_end','si_lead_followup_hook_settings_tab_footer');#for AB Solution high version V2.8.4
...
function si_lead_followup_hook_admin_init()
{
	...
	if (is_admin() || has_permission('settings', '', 'view')) {
		$CI->app_tabs->add_settings_tab('si_lead_followup_settings', [
			...
			'position' => 60,
			'icon'     => 'fa fa-calendar',//supported from AB Solutions V 3.0
		]);
	}
	if(get_option(SI_LEAD_FOLLOWUP_MODULE_NAME.'_activated') && get_option(SI_LEAD_FOLLOWUP_MODULE_NAME.'_activation_code')!=''){
