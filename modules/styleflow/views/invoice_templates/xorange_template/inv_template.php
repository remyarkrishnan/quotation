...
$info_left_column .= '<br><div style="background-color: orange;color: white"><span style="font-weight:bold;font-size:27px;">' . $invoice_number . '</span></div><br>';
...
// Bill to
$invoice_info = '<div style="background-color: orange; text-align: center; color: white; font-size: 12px !important;">';
...
$invoice_info .= '<br /><b style="color: orange">' . _l('invoice_data_date') . '</b> ' . _d($invoice->date) . '<br />';

$invoice_info = hooks()->apply_filters('invoice_pdf_header_after_date', $invoice_info, $invoice);

if (!empty($invoice->duedate)) {
    $invoice_info .= '<b style="color: orange">'._l('invoice_data_duedate') . '</b> ' . _d($invoice->duedate) . '<br />';
...
// The items table
$items = styleflow_get_items_table_data($invoice, 'xorange', 'pdf');
