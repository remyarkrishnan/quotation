...
//$info_right_column .= '<br><div style="background-color: orange;color: white;"><span style="font-weight:bold;font-size:27px;">' . $invoice_number . '</span></div><br>';
$info_right_column .= '<div style="  position: absolute;
  top: 50%;
  left: 50px;
  right: 50px;
  border-top: 5px solid #bfbf55;
  z-index: -1;"></div>';
$info_right_column .= '<b style="color:#4e4e4e;font-weight:bold;font-size:35px;"># ' . $invoice_number . '</b>';

$invoice_info = _d($invoice->date);
$invoice_info = hooks()->apply_filters('invoice_pdf_header_after_date', $invoice_info, $invoice);

if (!empty($invoice->duedate)) {
    $invoice_due_info = _d($invoice->duedate);
    $invoice_due_info = hooks()->apply_filters('invoice_pdf_header_after_due_date', $invoice_due_info, $invoice);
}


if (get_option('show_status_on_pdf_ei') == 1) {
    $inv_status_html = '<br /><span style="color:rgb(' . invoice_status_color_pdf($status) . ');text-transform:uppercase;">' . format_invoice_status($status, '', false) . '</span>';
}
...
<br>
    <table style="width: 100%; border-collapse: collapse; border: 1px solid #ccc;">
        <tr>
            <th style="padding: 10px; border: 1px solid #ccc; background-color: #bfbf55;color: white;font-weight: bold">'._l('invoice_data_date').'</th>
            <th style="padding: 10px; border: 1px solid #ccc; background-color: #bfbf55;color: white;font-weight: bold">'._l('invoice_data_duedate').'</th> ';
if (get_option('show_status_on_pdf_ei') == 1) {
    $info_right_column .= '
            <th style="padding: 10px; border: 1px solid #ccc; background-color: #bfbf55;color: white;font-weight: bold">' . _l('clients_invoice_dt_status') . '</th> ';
}
$info_right_column .='
        </tr>
        <tr>
            <td style="padding: 10px; border: 1px solid #ccc;">'.$invoice_info.'</td>
            <td style="padding: 10px; border: 1px solid #ccc;">'.$invoice_due_info.'</td>';
if (get_option('show_status_on_pdf_ei') == 1) {
    $info_right_column .= '<td style="padding: 10px; border: 1px solid #ccc;">' . $inv_status_html . '</td> ';
}
$info_right_column .='
        </tr>
    </table>
...
// Bill to
$invoice_info = '<b style="color:#bfbf55;">' . _l('invoice_bill_to') . ':</b><br>';
...
if ($invoice->include_shipping == 1 && $invoice->show_shipping_on_invoice == 1) {
    $invoice_info .= '<br /><b style="color: #bfbf55">' . _l('ship_to') . ':</b>';
...
if ($invoice->sale_agent != 0 && get_option('show_sale_agent_on_invoices') == 1) {
    $invoice_info .= '<b style="color: #bfbf55">'._l('sale_agent_string') . '</b>: ' . get_staff_full_name($invoice->sale_agent) . '<br />';
...
if ($invoice->project_id != 0 && get_option('show_project_on_invoice') == 1) {
    $invoice_info .= '<b style="color: #bfbf55">'._l('project') . '</b>: ' . get_project_name_by_id($invoice->project_id) . '<br />';
...
// The items table
$items = styleflow_get_items_table_data($invoice, 'sysyellow', 'pdf');
...
$tbltotal .= '
<tr style="background-color: #bfbf55; color: white">
...
if (count($invoice->payments) > 0 && get_option('show_transactions_on_invoice_pdf') == 1) {
    $pdf->Ln(4);
    $border = 'border-bottom-color:#d7bb04;border-bottom-width:1px;border-bottom-style:solid; 1px solid #d7bb04;';
    $pdf->SetFont($font_name, 'B', $font_size);
    $pdf->Cell(0, 0, _l('invoice_received_payments') . ':', 0, 1, 'L', 0, '', 0);
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(4);
    $tblhtml = '<table width="100%" bgcolor="#fff" cellspacing="0" cellpadding="5" border="0">
        <tr height="20"  style="color:#d7bb04;border:1px solid #d7bb04;">
        <th width="25%;" style="background-color: #bfbf55;color: white;' . $border . '">' . _l('invoice_payments_table_number_heading') . '</th>
        <th width="25%;" style="background-color: #bfbf55;color: white;' . $border . '">' . _l('invoice_payments_table_mode_heading') . '</th>
        <th width="25%;" style="background-color: #bfbf55;color: white;' . $border . '">' . _l('invoice_payments_table_date_heading') . '</th>
        <th width="25%;" style="background-color: #bfbf55;color: white;' . $border . '">' . _l('invoice_payments_table_amount_heading') . '</th>
    </tr>';
    $tblhtml .= '<tbody>';
