...
$info_left_column .= '<br><div style="background-color: #bfbf55;color: white"><span style="font-weight:bold;font-size:27px;">' . $invoice_number . '</span></div><br>';
//$info_right_column .= '<b style="color:#4e4e4e;"># ' . $invoice_number . '</b>';

if (get_option('show_status_on_pdf_ei') == 1) {
    $info_left_column .= '<br /><span style="color:rgb(' . invoice_status_color_pdf($status) . ');text-transform:uppercase;">' . format_invoice_status($status, '', false) . '</span>';
}
...
if ($status != Invoices_model::STATUS_PAID && $status != Invoices_model::STATUS_CANCELLED && get_option('show_pay_link_to_invoice_pdf') == 1
    ...
    $info_left_column .= ' - <a style="color:#84c529;text-decoration:none;text-transform:uppercase;" href="' . site_url('invoice/' . $invoice->id . '/' . $invoice->hash) . '"><1b>' . _l('view_invoice_pdf_link_pay') . '</1b></a>';
}
// Add logo
...
// Bill to
$invoice_info = '<div style="background-color: #bfbf55; text-align: center; color: white; font-size: 12px !important;">';
...
$invoice_info .= '<br /><b style="color: #bfbf55">' . _l('invoice_data_date') . '</b> ' . _d($invoice->date) . '<br />';

$invoice_info = hooks()->apply_filters('invoice_pdf_header_after_date', $invoice_info, $invoice);

if (!empty($invoice->duedate)) {
    $invoice_info .= '<b style="color: #bfbf55">'._l('invoice_data_duedate') . '</b> ' . _d($invoice->duedate) . '<br />';
...
// The items table
$items = styleflow_get_items_table_data($invoice, 'xyellow', 'pdf');
